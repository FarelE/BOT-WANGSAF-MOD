...
async function imageToWebp(media) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            ...
            "libwebp",
            "-vf",
            "scale='scale=512:512:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1"
        ])
            .toFormat("webp")
            .save(tmpFileOut);
    });
...
async function videoToWebp(media) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            ...
            "libwebp",
            "-vf",
            "scale='scale='scale=512:512:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1",
            "-loop",
            "0",
            "-ss",
            "00:00:00",
