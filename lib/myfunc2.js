...
const { jidDecode, proto, getContentType, downloadContentFromMessage } = baileys;
...
export const smsg = (sock, m, store) => {
	...
	*/
    m.reply = (text, chatId, options) => sock.sendMessage(chatId ? chatId : m.chat, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'),showAdAttribution: true }, ...options }, { quoted:m })
    /**
	* Copy this message
	*/
