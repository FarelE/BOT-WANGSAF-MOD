...
async function imageToWebpreso(media, reso) {
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            ...
            "libwebp",
            "-vf",
            `scale='min(${reso},iw)':min'(${reso},ih)':force_original_aspect_ratio=decrease,fps=15, pad=${reso}:${reso}:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`
        ])
            .toFormat("webp")
            .save(tmpFileOut);
    });
